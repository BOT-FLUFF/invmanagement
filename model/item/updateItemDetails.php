...
if (isset($_POST['itemNumber'])) {
    ...
    if (!empty($itemNumber) && !empty($itemName) && isset($itemDetailsQuantity) && isset($itemDetailsUnitPrice) && !empty($itemMan) && !empty($itemExp)) {
        ...
        if (!preg_match($dateRegex, $itemMan) || !preg_match($dateRegex, $itemExp)) {
            $errorAlert = '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Please enter valid dates in YYYY-MM-DD format for manufacturing and expiration dates</div>';
            $data = ['alertMessage' => $errorAlert];
            echo json_encode($data);
            exit();
        }
        ...
        $updateItemDetailsSql = 'UPDATE item SET itemName = :itemName, discount = :discount, stock = :stock, unitPrice = :unitPrice, status = :status, description = :description, itemMan = :itemMan, itemExp = :itemExp WHERE itemNumber = :itemNumber';
        ...
        $updateItemInSaleTableStatement = $conn->prepare($updateItemInSaleTableSql);
        $updateItemInSaleTableStatement->execute(['itemName' => $itemName, 'itemNumber' => $itemNumber]);
        ...
        $updateItemInPurchaseTableStatement = $conn->prepare($updateItemInPurchaseTableSql);
        $updateItemInPurchaseTableStatement->execute(['itemName' => $itemName, 'itemNumber' => $itemNumber]);
